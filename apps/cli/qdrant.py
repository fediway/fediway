...
def create_embeddings(batch_size: int = 16):
    ...
    with db_session() as db:
        query_select = (
            ...
            .where(Status.created_at > config.fediway.feed_max_age())
        )
        query_count = (
            ...
            if len(batch) >= batch_size:
                ...
                client.upsert(
                    ...
                    points=models.Batch(
                        ...
                        vectors=embeddings,
                        payloads=[{'created_at': status['created_at'].timestamp()} for status in batch],
                    )
                )
...
def collect(language: str = 'en'):
    ...
    from modules.fediway.sources.qdrant import (
        SimilarToFavourited
    )
