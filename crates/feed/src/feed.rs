...
/// Result of a feed execution with metadata for observability.
pub struct FeedResult<Item> {
...
impl<Item: Send + Sync + 'static, Ctx: Send + Sync + 'static> Feed<Item, Ctx> {
    #[must_use]
    pub fn builder() -> FeedBuilder<Item, Ctx> {
        FeedBuilder::new()
    }

    pub async fn execute(&self, limit: usize, ctx: &Ctx) -> FeedResult<Item> {
        let start = Instant::now();

        let mut candidates = self.collect().await;
        let collected = candidates.len();
        info!(feed = self.name, count = collected, "candidates collected");

        for filter in &self.filters {
            filter.apply(&mut candidates, ctx);
        }
        let filtered = candidates.len();
        info!(feed = self.name, count = filtered, "after filtering");
        ...
        let futures: Vec<_> = self
            ...
            .map(|(source, limit)| {
                ...
                async move {
                    ...
                    observe::source_collected(feed_name, name, batch.len(), start.elapsed());
                    info!(
                        feed = feed_name,
                        source = name,
                        count = batch.len(),
...
pub struct FeedBuilder<Item: Send, Ctx = ()> {
    name: &'static str,
    sources: Vec<(Box<dyn Source<Item>>, usize)>,
    filters: Vec<Box<dyn Filter<Item, Ctx>>>,
    scorers: Vec<Box<dyn Scorer<Item, Ctx>>>,
...
impl<Item: Send + Sync + 'static, Ctx: Send + Sync + 'static> FeedBuilder<Item, Ctx> {
    fn new() -> Self {
        Self {
            name: "unnamed",
            sources: Vec::new(),
    ...
    #[must_use]
    pub fn build(self) -> Feed<Item, Ctx> {
        Feed {
            name: self.name,
