...
def test_followed_by_friends_source_has_required_attributes():
    mock_rw = MagicMock()
    source = FollowedByFriendsSource(rw=mock_rw, account_id=1)

    assert source.id == "followed_by_friends"
    assert "min_mutual_follows" in source._tracked_params


def test_followed_by_friends_source_get_params():
    mock_rw = MagicMock()
    source = FollowedByFriendsSource(
        rw=mock_rw,
        account_id=1,
...
def test_followed_by_friends_source_collect_executes_query():
    mock_rw = MagicMock()
    ...
    source = FollowedByFriendsSource(rw=mock_rw, account_id=1, min_mutual_follows=2)
...
def test_followed_by_friends_source_collect_returns_empty_on_no_results():
    mock_rw = MagicMock()
    mock_rw.execute.return_value.fetchall.return_value = []
...
def test_followed_by_friends_source_respects_limit():
    mock_rw = MagicMock()
