...
class Recommendation:
    ...
    def __init__(
        self, item: str | int, score: float, adjusted_score: float, sources: list[str]
    ):
        self.item = item
...
class Feed:
    ...
    def merge_dict(self, data):
        self.id = data["id"]
        self.heuristics = [
            h.from_dict(data) for h, data in zip(self.heuristics, data["heuristics"])
        ]
        self.seen_candidates = set(data["seen_candidates"])
        self.candidate_queues = [
            TopKPriorityQueue.from_dict(q) for q in data["candidate_queues"]
        ]
    ...
    async def rank(
        self, candidates: list[int | str] | None = None, ranker_index: int = 0
    ):
        ...
        while True:
            ...
            for heuristic in self.heuristics:
                heuristic.update_seen(
                    candidate,
                    self.features.get([{"status_id": candidate}], heuristic.features)[
                        0
                    ],
                )
