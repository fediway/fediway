...
class TopFollowsSource(Source):
    ...
    def _get_affinities(self) -> dict[int, dict]:
        query = text("""
            SELECT followed_id, direct_affinity, inferred_affinity,
    ...
    def _get_recent_posts(self, limit: int):
        query = text("""
            WITH author_volume AS (
                SELECT account_id, COUNT(*) as posts_24h
                FROM statuses
            ...
            SELECT
                ...
                s.created_at,
                COALESCE(av.posts_24h, 0) AS author_posts_24h
            FROM statuses s
            JOIN follows f ON f.target_account_id = s.account_id
            LEFT JOIN author_volume av ON av.account_id = s.account_id
            WHERE f.account_id = :user_id
              AND s.created_at > NOW() - INTERVAL :max_age_hours HOUR
    ...
    def _score_posts(self, posts, affinities, max_affinity):
        ...
        for row in posts:
            status_id, author_id, created_at, author_posts_24h = row
            ...
            volume_penalty = min(1.0, self.volume_threshold / max(author_posts_24h, 1))
