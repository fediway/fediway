...
{% for window_size, spec in [('24 HOURS', '1d'), ('7 DAYS', '7d'), ('60 DAYS', '60d')] %}
  CREATE TABLE IF NOT EXISTS offline_features_account_instance_engagement_{{ spec }} (
    account_id BIGINT,
    instance VARCHAR,
    ...
    avg_mentions FLOAT,
    avg_tags FLOAT,
    PRIMARY KEY (account_id, instance, event_time)
  ) APPEND ONLY ON CONFLICT IGNORE WITH (
    connector='kafka',
    topic='offline_features_account_instance_engagement_{{ spec }}',
...
{% for spec in ['1d', '7d', '60d'] -%}
  DROP TABLE IF EXISTS offline_features_account_instance_engagement_{{ spec }};
{% endfor -%}
