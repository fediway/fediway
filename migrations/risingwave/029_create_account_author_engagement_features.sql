...
{% for window_size, spec in [('24 HOURS', '1d'), ('7 DAYS', '7d'), ('60 DAYS', '60d')] %}
  CREATE MATERIALIZED VIEW IF NOT EXISTS online_features_account_author_engagement_{{ spec }} AS
  SELECT
  ...
  -- compute account-author features only for users on our instance
  AND EXISTS (SELECT 1 FROM users u WHERE u.account_id = e.account_id) 
  GROUP BY e.account_id, e.author_id;
  
  CREATE SINK IF NOT EXISTS online_features_account_author_engagement_{{ spec }}_sink
  FROM online_features_account_author_engagement_{{ spec }}
  WITH (
    connector='kafka',
    properties.bootstrap.server='{{ bootstrap_server }}',
    topic='online_features_account_author_engagement_{{ spec }}',
    primary_key='account_id,author_id',
    properties.linger.ms='10000',
  ) FORMAT PLAIN ENCODE JSON (
    force_append_only='true'
  );

  CREATE TABLE IF NOT EXISTS offline_features_account_author_engagement_{{ spec }} (
    account_id BIGINT,
    author_id BIGINT,
    ...
    avg_mentions FLOAT,
    avg_tags FLOAT,
    PRIMARY KEY (account_id, author_id, event_time)
  ) APPEND ONLY ON CONFLICT IGNORE WITH (
    connector='kafka',
    topic='offline_features_account_author_engagement_{{ spec }}',
    properties.bootstrap.server='{{ bootstrap_server }}',
  ) FORMAT PLAIN ENCODE JSON;
{% endfor %}
...
{% for spec in ['1d', '7d', '60d'] -%}
  DROP VIEW IF EXISTS online_features_account_author_engagement_{{ spec }};
  DROP SINK IF EXISTS online_features_account_author_engagement_{{ spec }}_sink;
  DROP TABLE IF EXISTS offline_features_account_author_engagement_{{ spec }};
{% endfor -%}