...
{% for window_size, spec in [('24 HOURS', '1d'), ('7 DAYS', '7d'), ('60 DAYS', '60d')] %}
  CREATE MATERIALIZED VIEW IF NOT EXISTS online_features_account_domain_engagement_{{ spec }} AS
  SELECT
  ...
  CREATE TABLE IF NOT EXISTS offline_features_account_domain_engagement_{{ spec }} (
    account_id BIGINT,
    domain VARCHAR,
    ...
    avg_mentions FLOAT,
    avg_tags FLOAT,
    PRIMARY KEY (account_id, domain, event_time)
  ) APPEND ONLY ON CONFLICT IGNORE WITH (
    connector='kafka',
    topic='offline_features_account_domain_engagement_{{ spec }}',
    properties.bootstrap.server='{{ bootstrap_server }}',
  ) FORMAT PLAIN ENCODE JSON;
{% endfor %}
...
{% for spec in ['1d', '7d', '60d'] -%}
  DROP VIEW IF EXISTS online_features_account_domain_engagement_{{ spec }};
  DROP SINK IF EXISTS online_features_account_domain_engagement_{{ spec }}_sink;
  DROP TABLE IF EXISTS offline_features_account_domain_engagement_{{ spec }};
{% endfor -%}