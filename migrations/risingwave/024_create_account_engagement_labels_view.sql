...
SELECT
    ...
    -- is_quoted
    e.type = 5 as is_quoted,
    CASE WHEN e.type = 5 THEN e.event_time END AS is_quoted_at,

    -- is_reply_or_quote_engaged_by_author
    author_e.status_id IS NOT NULL as is_reply_or_quote_engaged_by_author,
    author_e.event_time AS is_reply_or_quote_engaged_by_author_at

FROM enriched_status_engagement_events e

-- for is_reply_engaged_by_author and  label
LEFT JOIN (
...
AND e.type IN (2, 5)
AND author_e.status_id = e.entity_id
