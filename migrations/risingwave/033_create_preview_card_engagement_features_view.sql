...
{% for window_size, spec in [('24 HOURS', '1d'), ('7 DAYS', '7d'), ('60 DAYS', '60d')] %}
  CREATE MATERIALIZED VIEW IF NOT EXISTS online_features_preview_card_engagement_{{ spec }} AS
  ...
  SELECT
      ...
      APPROX_COUNT_DISTINCT(target_domain) as num_target_domain,
  ...
  GROUP BY p.preview_card_id;
  
  CREATE SINK IF NOT EXISTS online_features_preview_card_engagement_{{ spec }}_sink
  FROM online_features_preview_card_engagement_{{ spec }}
  WITH (
    connector='kafka',
    properties.bootstrap.server='{{ bootstrap_server }}',
    topic='online_features_preview_card_engagement_{{ spec }}',
    primary_key='preview_card_id',
    properties.linger.ms='10000',
  ) FORMAT PLAIN ENCODE JSON (
    force_append_only='true'
  );

  CREATE TABLE IF NOT EXISTS offline_features_preview_card_engagement_{{ spec }} (
    ...
    avg_mentions FLOAT,
    avg_tags FLOAT,
    PRIMARY KEY (preview_card_id, event_time)
  ) APPEND ONLY ON CONFLICT IGNORE WITH (
    connector='kafka',
    topic='offline_features_preview_card_engagement_{{ spec }}',
    properties.bootstrap.server='{{ bootstrap_server }}',
  ) FORMAT PLAIN ENCODE JSON;
{% endfor %}
...
{% for spec in ['1d', '7d', '56d'] %}
    DROP SINK IF EXISTS online_features_preview_card_engagement_{{ spec }}_sink;
    DROP VIEW IF EXISTS online_features_preview_card_engagement_{{ spec }};
    DROP TABLE IF EXISTS offline_features_preview_card_engagement_{{ spec }};
{% endfor %}