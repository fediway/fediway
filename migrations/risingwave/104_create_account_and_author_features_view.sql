...
{% for group_id, group, interval, interval_name, specs in [
  ...
  SELECT
      ...
      {% for intervals, spec in specs %}
        SUM(num_images) OVER (PARTITION BY {{ group_id }} ORDER BY window_end ROWS BETWEEN {{ intervals }} PRECEDING AND CURRENT ROW) AS num_images_{{ spec }},
        SUM(num_gifvs) OVER (PARTITION BY {{ group_id }} ORDER BY window_end ROWS BETWEEN {{ intervals }} PRECEDING AND CURRENT ROW) AS num_gifvs_{{ spec }},
        SUM(num_videos) OVER (PARTITION BY {{ group_id }} ORDER BY window_end ROWS BETWEEN {{ intervals }} PRECEDING AND CURRENT ROW) AS num_videos_{{ spec }},
        SUM(num_audios) OVER (PARTITION BY {{ group_id }} ORDER BY window_end ROWS BETWEEN {{ intervals }} PRECEDING AND CURRENT ROW) AS num_audios_{{ spec }},
        SUM(fav_count) OVER (PARTITION BY {{ group_id }} ORDER BY window_end ROWS BETWEEN {{ intervals }} PRECEDING AND CURRENT ROW) AS fav_count_{{ spec }},
        SUM(reblogs_count) OVER (PARTITION BY {{ group_id }} ORDER BY window_end ROWS BETWEEN {{ intervals }} PRECEDING AND CURRENT ROW) AS reblogs_count_{{ spec }},
        SUM(replies_count) OVER (PARTITION BY {{ group_id }} ORDER BY window_end ROWS BETWEEN {{ intervals }} PRECEDING AND CURRENT ROW) AS replies_count_{{ spec }}
...
-- :down
{% for group, interval_name in [('account', 'hourly'), ('author', 'hourly')] -%}
